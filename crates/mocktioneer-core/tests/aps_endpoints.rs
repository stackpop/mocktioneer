use mocktioneer_core::aps::{ApsBidRequest, ApsSlot};
use mocktioneer_core::auction::{build_aps_response, FIXED_BID_CPM};

#[cfg(test)]
mod tests {
    use super::{build_aps_response, FIXED_BID_CPM};
    use super::{ApsBidRequest, ApsSlot};

    #[test]
    fn build_aps_response_single_slot() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "header-banner".to_owned(),
                sizes: vec![[300, 250]],
                slot_name: Some("header-banner".to_owned()),
            }],
            page_url: Some("https://example.com".to_owned()),
            user_agent: Some("Mozilla/5.0".to_owned()),
            timeout: Some(800),
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.status, Some("ok".to_owned()));
        assert_eq!(resp.contextual.slots.len(), 1);

        let slot = &resp.contextual.slots[0];
        assert_eq!(slot.slot_id, "header-banner");
        assert_eq!(slot.size, "300x250");
        assert_eq!(slot.media_type, Some("d".to_owned()));
        assert_eq!(slot.fif, Some("1".to_owned()));
        assert!(slot.crid.is_some());

        assert!(slot.amzniid.is_some());
        assert!(slot.amznbid.is_some());
        assert!(slot.amznp.is_some());
        assert_eq!(slot.amznsz, Some("300x250".to_owned()));
        assert_eq!(slot.amznactt, Some("OPEN".to_owned()));

        assert!(slot.targeting.contains(&"amzniid".to_owned()));
        assert!(slot.targeting.contains(&"amznbid".to_owned()));
        assert!(slot.targeting.contains(&"amznsz".to_owned()));

        assert!(slot.meta.contains(&"slotID".to_owned()));
        assert!(slot.meta.contains(&"mediaType".to_owned()));
        assert!(slot.meta.contains(&"size".to_owned()));
    }

    #[test]
    fn build_aps_response_multiple_slots() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![
                ApsSlot {
                    slot_id: "header".to_owned(),
                    sizes: vec![[728, 90]],
                    slot_name: Some("header".to_owned()),
                },
                ApsSlot {
                    slot_id: "sidebar".to_owned(),
                    sizes: vec![[300, 250]],
                    slot_name: Some("sidebar".to_owned()),
                },
                ApsSlot {
                    slot_id: "footer".to_owned(),
                    sizes: vec![[970, 250]],
                    slot_name: Some("footer".to_owned()),
                },
            ],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 3);
        assert_eq!(resp.contextual.slots[0].slot_id, "header");
        assert_eq!(resp.contextual.slots[0].size, "728x90");
        assert_eq!(resp.contextual.slots[1].slot_id, "sidebar");
        assert_eq!(resp.contextual.slots[1].size, "300x250");
        assert_eq!(resp.contextual.slots[2].slot_id, "footer");
        assert_eq!(resp.contextual.slots[2].size, "970x250");

        for slot in &resp.contextual.slots {
            assert!(slot.amznbid.is_some());
            assert!(slot.amznp.is_some());
        }
    }

    #[test]
    fn build_aps_response_nonstandard_size_skipped() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "custom-slot".to_owned(),
                sizes: vec![[333, 222]],
                slot_name: Some("custom-slot".to_owned()),
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 0);
    }

    #[test]
    fn build_aps_response_mixed_sizes() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![
                ApsSlot {
                    slot_id: "standard".to_owned(),
                    sizes: vec![[300, 250]],
                    slot_name: Some("standard".to_owned()),
                },
                ApsSlot {
                    slot_id: "nonstandard".to_owned(),
                    sizes: vec![[333, 222]],
                    slot_name: Some("nonstandard".to_owned()),
                },
            ],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
        assert_eq!(resp.contextual.slots[0].slot_id, "standard");
    }

    #[test]
    fn aps_targeting_structure() {
        let req = ApsBidRequest {
            pub_id: "test".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "slot1".to_owned(),
                sizes: vec![[728, 90]],
                slot_name: None,
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "test.host", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
        let slot = &resp.contextual.slots[0];

        assert!(slot.amzniid.is_some());
        assert!(slot.amznbid.is_some());
        assert!(slot.amznp.is_some());
        assert!(slot.amznsz.is_some());
        assert!(slot.amznactt.is_some());

        let impression_id = slot.amzniid.as_ref().unwrap();
        assert!(!impression_id.is_empty());

        let encoded_price = slot.amznbid.as_ref().unwrap();
        assert!(!encoded_price.is_empty());

        assert_eq!(slot.amznsz.as_ref().unwrap(), "728x90");

        assert_eq!(slot.amznactt.as_ref().unwrap(), "OPEN");
    }

    #[test]
    fn aps_bid_multiple_sizes_per_slot() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "multi-size".to_owned(),
                sizes: vec![[728, 90], [970, 250]],
                slot_name: Some("multi-size".to_owned()),
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
        assert_eq!(resp.contextual.slots[0].size, "970x250");
    }

    #[test]
    fn aps_bid_non_standard_first_then_standard() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "mixed-slot".to_owned(),
                sizes: vec![[999, 999], [300, 250]],
                slot_name: Some("mixed-slot".to_owned()),
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
        assert_eq!(resp.contextual.slots[0].size, "300x250");
    }

    #[test]
    fn aps_bid_selects_largest_area_from_multiple_standard_sizes() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "multi-standard".to_owned(),
                sizes: vec![[320, 50], [300, 250], [970, 250]],
                slot_name: Some("multi-standard".to_owned()),
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
        assert_eq!(resp.contextual.slots[0].size, "970x250");
    }

    #[test]
    fn aps_bid_all_non_standard_sizes_skips_slot() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "all-non-standard".to_owned(),
                sizes: vec![[999, 999], [888, 888], [777, 777]],
                slot_name: Some("all-non-standard".to_owned()),
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 0);
    }

    #[test]
    fn aps_response_empty_slots() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 0);
        assert_eq!(resp.contextual.status, Some("ok".to_owned()));
    }

    #[test]
    fn aps_contextual_metadata() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "test-slot".to_owned(),
                sizes: vec![[300, 250]],
                slot_name: None,
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        let contextual = &resp.contextual;
        assert_eq!(contextual.status, Some("ok".to_owned()));
        assert_eq!(contextual.cfe, Some(true));
        assert_eq!(contextual.ev, Some(true));
        assert!(contextual.host.is_some());
        assert!(contextual
            .host
            .as_ref()
            .unwrap()
            .contains("mocktioneer.test"));
        assert_eq!(contextual.cb, Some("6".to_owned()));
        assert_eq!(
            contextual.cfn,
            Some("bao-csm/direct/csm_othersv6.js".to_owned())
        );
    }

    #[test]
    fn aps_response_no_adm_field() {
        let req = ApsBidRequest {
            pub_id: "5555".to_owned(),
            slots: vec![ApsSlot {
                slot_id: "test-slot".to_owned(),
                sizes: vec![[300, 250]],
                slot_name: None,
            }],
            page_url: None,
            user_agent: None,
            timeout: None,
        };

        let resp = build_aps_response(&req, "mocktioneer.test", FIXED_BID_CPM);

        assert_eq!(resp.contextual.slots.len(), 1);
    }
}
