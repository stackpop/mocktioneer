...
fn ec_sync_endpoints_are_routed() {
    ...
    let sync_start = block_on(app.router().oneshot(make_request(
        Method::GET,
    ...
    let sync_done = block_on(app.router().oneshot(make_request(
        Method::GET,
        "/sync/done?ts_synced=1",
        Body::empty(),
    )));
    ...
    let resolve = block_on(app.router().oneshot(make_request(
        Method::GET,
